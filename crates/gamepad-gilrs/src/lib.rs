...
use gilrs::{Error, EventType, Gamepad, GamepadId, Gilrs};
use limnus_app::prelude::{App, Plugin};
use limnus_gamepad::{Axis, Button, Gamepads};
...
impl GamepadGilrs {
    ...
    pub fn tick(&mut self, gamepads: &mut Gamepads) {
            ...
            match event.event {
                ...
                EventType::ButtonChanged(gilrs_button, button_value, _) => {
                    if let Some(real_button) = convert_button(&gilrs_button) {
                        gamepads.set_button(event.id.into(), real_button, button_value);
                    }
                }
                EventType::AxisChanged(gilrs_axis, axis_value, _) => {
                    if let Some(real_axis) = convert_axis(&gilrs_axis) {
                        gamepads.set_axis(event.id.into(), real_axis, axis_value);
                    }
                }
                EventType::Connected => {
                    let gamepad = self.gilrs.gamepad(event.id);
                    gamepads.connected(event.id.into(), gamepad.name());
                }
                EventType::Disconnected => {
                    gamepads.disconnected(event.id.into());
                }
                EventType::Dropped => {}
...
fn check_gamepads(mut gilrs: LoReM<GamepadGilrs>, mut gamepads: ReM<Gamepads>) {
    gilrs.tick(&mut gamepads);
}
